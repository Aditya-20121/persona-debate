          ...
          >
            <div className="mt-2 space-y-2">
              {chunks.map((chunk, i) => (
                <div
                  key={i}
                  className="rounded-lg bg-black/20 border border-arena-border p-3 text-xs leading-relaxed"
                >
                  <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                    {chunk.ethical_dilemma_type &&
                      chunk.ethical_dilemma_type !== "None" && (
                        <span className="px-1.5 py-0.5 rounded bg-white/15 text-white/90 font-medium">
                          {chunk.ethical_dilemma_type}
                        </span>
                      )}
                    {chunk.topic_keywords?.slice(0, 4).map((kw, j) => (
                      <span
                        key={j}
                        className="px-1.5 py-0.5 rounded bg-white/[0.07] text-muted"
                      >
                        {kw}
                      </span>
                    ))}
                  ...
                  {chunk.philosophical_summary && (
                    <p className="italic text-white/80 mb-1.5">
                      "{chunk.philosophical_summary}"
                    </p>
                  )}

                  <p className="text-muted line-clamp-4">{chunk.content}</p>
                </div>
              ))}
            </div>
          </motion.div>
